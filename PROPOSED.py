...
# Enhanced Fire Hawk Optimizer (EFHO)
# Position update is done on the line 48
def PROPOSED(Pop, CostFunction, VarMin, VarMax, MaxFes):
    global BestCosts
    ...
    while FEs < MaxFes:
        ...
        for i in range(len(Pop2)):
            ...
            # Update position using the below formula
            # Ir = np.random.uniform(0, 1, size=(2,))
            r = (np.min(Cost) / np.max(Cost)) * 0.5
            Ir = np.asarray([r, r])
